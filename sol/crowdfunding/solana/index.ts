...
const keys = ["amount", "u64"] as const;
type KeyType = typeof keys[number];

type MapType<TClassName, FieldType> = Map<
  TClassName,
  {
    kind: "struct";
    fields: FieldType[][];
...
class WithdrawRequest {
  #amount: number;
  constructor(properties: { amount: number }) {
    //   this.#name = properties.name
    // const kk: KeyType[] = Object.keys(properties) as KeyType[];
    this.#amount = properties.amount;
    //    kk.forEach((key) => {
    //       (this as typeof properties)[key] = properties[key];
    //     });
  }

  static schema: MapType<typeof WithdrawRequest, KeyType> = new Map([
    [
      WithdrawRequest,
      {
        kind: "struct",
...
export async function withdraw(campaignPubKey: string, amount: number) {
  await checkWallet();
